...
window.openSkewtModal = function () {
    ...
    // ═══ PRIMER: Comprovar perfil precarregat ═══
    if (window._skewtPerfilPrecarregat) {
        ...
        puntActual = { 
            lat: punt.lat, 
            lon: punt.lon, 
            hourIdx: punt.hourIdx,
            horaItem: { dateObj: new Date(), data: { step: punt.hourIdx } },
            pobleProper 
        };
    ...
    if (!pos) {
        mostrarError('No hi ha cap punt seleccionat. Fes clic sobre el mapa primer.');
        return;
    }
