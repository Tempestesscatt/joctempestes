...
// ═══════════════════════════════════════════════════════════════════════
//  INICIALITZACIÓ - CÀRREGA MASSIVA AUTOMÀTICA
//  Aquest fitxer (mapa2.js) carrega TOTES les hores de cop a l'inici.
// ═══════════════════════════════════════════════════════════════════════

async function inicialitzarCarregaSotaDemanda() {
    // 1. Detectar quines hores existeixen
    ...
    // 3. Mostrar la graella de seguida
    construirGraellaHores();
    ...
    // 5. Carregar la primera hora ràpid per mostrar alguna cosa
    console.log('[Càrrega MASSIVA] Carregant primera hora per mostrar...');
    ...
    // 6. INICIAR CÀRREGA MASSIVA AUTOMÀTICA
    console.log('[Càrrega MASSIVA] Iniciant descàrrega de totes les hores...');
    iniciarCarregaMassivaAutomatica();
}
...
function construirGraellaHores() {
    ...
    totesLesHores.forEach((item, i) => {
        ...
        const estaCarregant = !!item._carregant;
        ...
        // 🔧 Colors segons estat: pendent (gris), carregant (blau), carregada (normal)
        let colorText, colorFons, colorBorder, opacitat;
        
        if (itemBloquejatVisual) {
            colorText = '#3a4a5a';
            colorFons = 'rgba(255,255,255,0.02)';
            colorBorder = '1px solid transparent';
            opacitat = '0.3';
        } else if (isActive) {
            colorText = '#FFD700';
            colorFons = 'rgba(255,215,0,0.12)';
            colorBorder = '1px solid rgba(255,215,0,0.3)';
            opacitat = '1';
        } else if (estaCarregant) {
            colorText = '#4a9eff';
            colorFons = 'rgba(74,158,255,0.15)';
            colorBorder = '1px solid rgba(74,158,255,0.4)';
            opacitat = '1';
        } else if (estaCarregada) {
            colorText = '#8899bb';
            colorFons = 'rgba(255,255,255,0.05)';
            colorBorder = '1px solid transparent';
            opacitat = '1';
        } else {
            colorText = '#556680';
            colorFons = 'rgba(255,255,255,0.03)';
            colorBorder = '1px solid transparent';
            opacitat = '0.55';
        }
...
// ═══════════════════════════════════════════════════════════════════════
//  INICIALITZACIÓ — ARRENCAR TOT
//  Aquesta crida detecta hores, crea esquelets, carrega la primera hora
//  i llança la càrrega massiva automàtica (vegeu iniciarCarregaMassivaAutomatica).
// ═══════════════════════════════════════════════════════════════════════

// ═══════════════════════════════════════════════════════════════════════
//  PRECÀRREGA MASSIVA — DESCARREGA TOTES LES HORES AL ARRENCAR
// ═══════════════════════════════════════════════════════════════════════

const CONCURRENCIA_PRECARREGA = 3;   // quantes hores en paral·lel
let _precarregaCancelada = false;

async function precarregarTotesLesHores() {
    if (!totesLesHores || totesLesHores.length === 0) return;

    const total = totesLesHores.length;
    let completades = 0;
    let fallades = 0;

    console.log(`[Precàrrega] Iniciant descàrrega de ${total} hores (concurrència ${CONCURRENCIA_PRECARREGA})...`);
    ...
    const actualitzarProgres = () => {
        const pct = Math.round((completades / total) * 100);
        if (barra) barra.style.width = pct + '%';
        if (text) text.textContent = `${completades} / ${total} hores (${pct}%)`;
    };
    actualitzarProgres();

    // 🔧 FIX: contador per refrescar la graella periòdicament
    let _ultimRefresc = 0;
    const REFRESC_INTERVAL = 3; // refrescar cada 3 hores carregades

    const refrescarGraellaSiCal = () => {
        if (completades - _ultimRefresc >= REFRESC_INTERVAL || completades === total) {
            _ultimRefresc = completades;
            if (typeof construirGraellaHores === 'function') {
                construirGraellaHores();
            }
        }
    };

    const cua = totesLesHores.map((item, i) => ({ idx: i, step: item.step }));

    async function treballador() {
        while (cua.length > 0 && !_precarregaCancelada) {
            const { idx, step } = cua.shift();

            if (totesLesHores[idx].data && totesLesHores[idx].data.variables) {
                completades++;
                actualitzarProgres();
                refrescarGraellaSiCal();
                continue;
            }

            // 🔧 Marcar com "carregant" visualment
            totesLesHores[idx]._carregant = true;

            try {
                const nou = await carregarUnStep(step, true);
                if (nou) {
                    totesLesHores[idx].dateObj = nou.dateObj;
                    totesLesHores[idx].data = nou.data;
                } else {
                    fallades++;
                    console.warn(`[Precàrrega] Hora ${idx} (step ${step}) no disponible`);
                }
            } catch (e) {
                fallades++;
                console.warn(`[Precàrrega] Error a hora ${idx}:`, e.message);
            }

            totesLesHores[idx]._carregant = false;
            completades++;
            actualitzarProgres();
            refrescarGraellaSiCal();
        }
    ...
    const treballadors = [];
    for (let i = 0; i < CONCURRENCIA_PRECARREGA; i++) {
        treballadors.push(treballador());
    }
    await Promise.all(treballadors);

    console.log(`[Precàrrega] ✅ Finalitzada: ${completades - fallades}/${total} ok, ${fallades} fallades`);

    // Refrescar la UI final amb totes les hores carregades
    construirGraellaHores();
    construirPanellParametres();
    ...
    if (typeof mostrarHora === 'function') {
        const idxActual = curIdx;
        const item = totesLesHores[idxActual];
        if (item && item.data) {
            canvasLayer._needsRedraw = true;
            canvasLayer.setData(item.data);
            actualitzarUIHora(idxActual);
        }
    }
}

// Sobreescriure la inicialització per fer precàrrega massiva
async function inicialitzarAmbPrecarregaMassiva() {
    // 1. Detectar hores
    const steps = await detectarHoresDisponibles();

    if (steps.length === 0) {
        console.warn('[Precàrrega] No s\'ha trobat cap hora disponible.');
        totesLesHores = [];
        window.totesLesHores = totesLesHores;
        construirGraellaHores();
        const loadingOverlay = document.getElementById('loading_overlay');
    ...
    // 3. Mostrar graella i panell (encara buits, però ja visibles)
    construirGraellaHores();
    construirPanellParametres();

    // 4. Carregar primera hora per tenir alguna cosa a pantalla
    const primerItem = await carregarUnStep(totesLesHores[0].step, true);
    if (primerItem) {
        totesLesHores[0].dateObj = primerItem.dateObj;
        totesLesHores[0].data = primerItem.data;
    ...
    // 5. 🔥 PRECÀRREGA MASSIVA DE TOTES LES ALTRES
    await precarregarTotesLesHores();
}


// ═══════════════════════════════════════════════════════════════════════
//  ESTIL PER AL PUNTET DE CÀRREGA DE LES HORES
// ═══════════════════════════════════════════════════════════════════════

(function injectarEstilCarregant() {
    if (document.getElementById('estil-carregant')) return;
    const style = document.createElement('style');
    style.id = 'estil-carregant';
    style.textContent = `
        @keyframes pulseCarregant {
...
// Engegar tot
inicialitzarAmbPrecarregaMassiva();
