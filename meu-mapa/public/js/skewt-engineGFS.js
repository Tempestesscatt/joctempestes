...
//
//  ── NOVETAT D'AQUESTA REVISIÓ ─────────────────────────────────────────
//  INTERPOLACIÓ EN log(p): el perfil retornat per extreurePerfil() ara
//  conté ~200 punts (pas ~5 hPa) interpolats logarítmicament entre els
//  nivells estàndard del model. Això fa que les línies de T i Td es
//  vegin CORBES i naturals al Skew-T, en lloc de segments rectes.
// ═══════════════════════════════════════════════════════════════════════
...
(function (global) {
     ...
     *
     * Aquesta versió:
     *  1. Llegeix els nivells estàndard del model (NIVELLS_PRESSIO),
     *     descartant els que queden per sota de la pressió superficial real.
     *  2. Ordena per pressió decreixent i elimina duplicats.
     *  3. INTERPOLA EN log(p) per obtenir un perfil fi (~5 hPa de pas)
     *     que fa que les corbes es vegin naturals al Skew-T.
     *  4. Calcula l'altitud (z) per a cada punt interpolat.
     */
    function extreurePerfil(data, lat, lon, opcions) {
        const opt = opcions || {};
        const usarPressioReal = opt.usarPressioReal !== false;
        ...
        // ── 1. Pressió superficial real (només com a llindar de tall) ──
        ...
        // ── 2. Nivells 3D (descartant els que estiguin per sota de pSfc) ──
        if (coord3d) {
        ...
        // ── 3. Ordenar per pressió decreixent i eliminar duplicats ──
        ...
        if (net.p.length < 2) return null;

        // ═══════════════════════════════════════════════════════════════
        // 4. INTERPOLAR EN log(p) PER OBTENIR UN PERFIL FI I SUAU
        // ═══════════════════════════════════════════════════════════════
        const DP_OBJECTIU = 5;  // hPa entre punts interpolats

        const perfil = { p: [], t: [], td: [], u: [], v: [] };

        for (let k = 0; k < net.p.length - 1; k++) {
            const pA = net.p[k], pB = net.p[k + 1];
            const tA = net.t[k], tB = net.t[k + 1];
            const tdA = net.td[k], tdB = net.td[k + 1];
            const uA = net.u[k], uB = net.u[k + 1];
            const vA = net.v[k], vB = net.v[k + 1];

            const dp = pA - pB;
            const nPassos = Math.max(2, Math.ceil(dp / DP_OBJECTIU));

            const logA = Math.log(pA);
            const logB = Math.log(pB);

            for (let s = 0; s < nPassos; s++) {
                const frac = s / nPassos;

                // Interpolació logarítmica de la pressió (CLAU!)
                const logP = logA + frac * (logB - logA);
                const p = Math.exp(logP);

                perfil.p.push(p);
                perfil.t.push(tA + frac * (tB - tA));
                perfil.td.push(tdA + frac * (tdB - tdA));
                perfil.u.push(uA + frac * (uB - uA));
                perfil.v.push(vA + frac * (vB - vA));
            }
        }

        // Afegir l'últim nivell
        const ultim = net.p.length - 1;
        perfil.p.push(net.p[ultim]);
        perfil.t.push(net.t[ultim]);
        perfil.td.push(net.td[ultim]);
        perfil.u.push(net.u[ultim]);
        perfil.v.push(net.v[ultim]);

        // ── 5. Calcular altituds ──
        perfil.z = perfil.p.map(pressioAAlcada);

        if (perfil.p.length < 3) return null;
        return perfil;
    }
