// ═══════════════════════════════════════════════════════════════════════
//  mapa.js — VERSIÓ AMB CAPES GEOJSON + GEO500 + T500 + ISOHIPSES
// ═══════════════════════════════════════════════════════════════════════
...
// GRUP PRINCIPAL (amb les noves variables)
...
const GRUPS_SIMPLES = {
    ...
    '500 hPa': ['geopotencial_500', 'temperatura_500'],'500 hPa': ['geopotencial_500', 'temperatura_500'],
...
// ═══════════════════════════════════════════════════════════════════════
//  STREAMLINES (VERSIÓ ORIGINAL COMPLETA)
// ═══════════════════════════════════════════════════════════════════════
...
// ═══════════════════════════════════════════════════════════════════════
//  DIBUIXAR ISOHIPSES (Geopotencial 500hPa) SOBRE temperatura 500hPa
// ═══════════════════════════════════════════════════════════════════════
...
function _dibuixarIsohipses() {
    ...
    // Només dibuixar isohipses si estem veient temperatura 500hPa
    if (variableActiva !== 'temperatura_500') return;

    const data = totesLesHores[curIdx].data;
    if (!data || !data.variables['geopotencial_500']) return;
...
// Algorisme simple de marching squares per extreure contorns
function marchingSquaresSimple(data, w, h, nivell) {
    ...
    for (let y = 0; y < h - 1; y++) {
        for (let x = 0; x < w - 1; x++) {
            ...
            // Vora superior
            if ((cas & 1) !== ((cas & 2) >> 1)) {
                const t = (nivell - v00) / (v10 - v00);
                puntsCela.push({ x: x + t, y: y });
            }
            // Vora dreta
            if ((cas & 2) !== ((cas & 4) >> 1)) {
                const t = (nivell - v10) / (v11 - v10);
                puntsCela.push({ x: x + 1, y: y + t });
            }
            // Vora inferior
            if (((cas & 4) >> 2) !== ((cas & 8) >> 3)) {
                const t = (nivell - v01) / (v11 - v01);
                puntsCela.push({ x: x + (1 - t), y: y + 1 });
            }
            // Vora esquerra
            if (((cas & 8) >> 3) !== (cas & 1)) {
                const t = (nivell - v00) / (v01 - v00);
                puntsCela.push({ x: x, y: y + t });
            }

            if (puntsCela.length >= 2) {
                punts.push(puntsCela[0]);
                punts.push(puntsCela[puntsCela.length - 1]);
            }
        }
    }
    return punts;
...
window.setStreamlineColor = function(color) { wCfg.streamlineColor = color; _redibuixarCanvasVent(); };
window.setStreamlineWidth = function(v) { wCfg.streamlineWidth = Math.min(3, Math.max(0.5, v)); _redibuixarCanvasVent(); };
window.setStreamlineOpacity = function(v) { wCfg.streamlineOpacity = Math.min(1, Math.max(0.1, v)); _redibuixarCanvasVent(); };
    ...
    _render: function() {
        ...
        for (let py = 0; py < size.y; py++) {
            for (let px = 0; px < size.x; px++) {
                ...
                for (let c = 0; c < 4; c++) {
                    const v = (1-ty)*((1-tx)*imgData.data[src00+c] + tx*imgData.data[src10+c]) + ty*((1-tx)*imgData.data[src01+c] + tx*imgData.data[src11+c]);
...
async function carregarTotsJSONs() {
    ...
    // Buscar quines hores existeixen REALMENT
    console.log('[Loading] Escanejant hores disponibles...');
    ...
    for (let i = 0; i < MAX_STEPS; i++) {
        ...
        try {
            ...
            if (resp.ok) {
                horesExistents.push(i);
                console.log('[Loading] Hora ' + horaStr + ' trobada');
            }
        } catch(e) {
            // 404 = no existeix, continuar
        }
    }
    
    if (horesExistents.length === 0) {
        console.error('[Loading] No s\'ha trobat cap hora a ' + DADES_PATH);
        return;
    }
    
    console.log('[Loading] ' + horesExistents.length + ' hores trobades: ' + horesExistents.join(', '));
    
    const total = horesExistents.length;
    actualitzarBarraProgress(0, total);
    
    // Carregar cada hora existent
    for (const i of horesExistents) {
    ...
    setTimeout(function() {
        if (totesLesHores && totesLesHores.length > 0) {
            ...
            console.log('[Skew-T] Dades publicades: ' + totesLesHores.length + ' hores disponibles');
        }
    }, 1000);
}
...
// ═══════════════════════════════════════════════════════════════════════
//  PANELL DE PARÀMETRES (resumit, mateixa lògica)
// ═══════════════════════════════════════════════════════════════════════
...
// ═══════════════════════════════════════════════════════════════════════
//  PANELL D'AJUSTOS (resumit)
// ═══════════════════════════════════════════════════════════════════════

function crearPanellAjustos() { /* mateix codi */ }
