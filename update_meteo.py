...
def main():
    print()
    print(f"  {CYN}{B}╔══════════════════════════════════════════════════════╗{R}")
    print(f"  {CYN}{B}║     TEMPESTES.CAT  —  DESCÀRREGA ERA5-Land           ║{R}")
    print(f"  {CYN}{B}╚══════════════════════════════════════════════════════╝{R}")
    print()
    ...
    lats_flat = np.round(lats_grid.flatten(), 4)
    lons_flat = np.round(lons_grid.flatten(), 4)

    total_punts  = len(lats_flat)
    total_chunks = (total_punts + CHUNK_SIZE - 1) // CHUNK_SIZE

    info("Model",          "ERA5-Land",                               CYN)
    info("Zona",           f"Catalunya  {LAT_MIN}–{LAT_MAX}°N  {LON_MIN}–{LON_MAX}°E", WHT)
    info("Període",        f"{START_DATE}  →  {END_DATE}",            YLW)
...
>>>>>>> deae97f7520762f70e0841d3fc77273a774e4bcf
    ...
    if failed:
        ...
        if repaired == len(failed):
            ...
            print(f"  {YLW}⚠  Reparats {repaired}/{len(failed)}{R}")
    else:
    ...
    # Obtenir timestamps del primer punt vàlid
    first_valid = next((v for v in results_dict.values() if v is not None), None)
    timestamps  = first_valid["hourly"]["time"] if first_valid else []
    n_hours     = len(timestamps)
    empty_arr   = [None] * n_hours
    ...
    final_json = {
        ...
        "hourly": {k: [] for k in VARS_SFC.values()},
    }
    ...
    for pt in range(total_punts):
        ...
        if loc is None or "hourly" not in loc:
            for jk in VARS_SFC.values():
                final_json["hourly"][jk].append(empty_arr[:])
            continue

        h = loc["hourly"]
        for ak, jk in VARS_SFC.items():
            arr = h.get(ak, empty_arr)
            final_json["hourly"][jk].append(
    ...
    js_path = os.path.join("web_data", "historic.js")
    print(f"  {D}Guardant {js_path}...{R}", end=" ", flush=True)
    try:
        with open(js_path, 'w', encoding='utf-8') as f:
            f.write("const HISTORIC_CAT = ")
    ...
    info("Hores totals",  str(n_hours),                               WHT)
